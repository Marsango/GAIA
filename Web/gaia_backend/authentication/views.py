...
def sync_usuario_by_cpf(request):
    ...
    try:
        ...
        # 📧 ENVIAR EMAILS DE NOTIFICAÇÃO (apenas para sync via CPF)
        try:
            config_email = ConfiguracaoEmail.objects.first()
            
            # Se CPF foi alterado, enviar email de notificação
            if 'cpf' in alteracoes:
                print(f"📧 Enviando email de notificação de mudança de login...", flush=True)
                assunto = config_email.assunto_mudanca_login if config_email else "GAIA - Seu Login foi Atualizado"
                mensagem_template = config_email.mensagem_mudanca_login if config_email else """Olá {nome},
...
Se você não realizou esta alteração, entre em contato imediatamente com nosso suporte."""
                ...
                try:
                    mensagem = mensagem_template.format(
                        ...
                        novo_login=usuario.cpf
                    )
                except:
                    mensagem = f"Seu novo login é: {usuario.cpf}"
                ...
                print(f"✅ Email enviado para {usuario.email}", flush=True)
            
            # Se email foi alterado, enviar email de confirmação para o novo email
            if 'email' in alteracoes:
                print(f"📧 Enviando email de confirmação para novo email...", flush=True)
                assunto = config_email.assunto_mudanca_email if config_email else "GAIA - Seu E-mail foi Atualizado"
                mensagem_template = config_email.mensagem_mudanca_email if config_email else """Olá {nome},
...
Se você não realizou esta alteração, entre em contato imediatamente com nosso suporte."""
                ...
                try:
                    ...
                    mensagem = f"Seu novo e-mail é: {usuario.email}"
                
                send_mail(
                    assunto,
                    mensagem,
                    settings.DEFAULT_FROM_EMAIL,
                    [usuario.email],  # Envia para o NOVO email
                    fail_silently=False
                )
                print(f"✅ Email de confirmação enviado para {usuario.email}", flush=True)
        except Exception as e:
            print(f"⚠️ Erro ao enviar email de notificação: {str(e)[:150]}", flush=True)
            # Não bloqueia a sincronização se email falhar
...
def sync_usuario_by_cnpj(request):
    ...
    try:
        ...
        # 📧 ENVIAR EMAILS DE NOTIFICAÇÃO (apenas para sync via CNPJ)
        try:
            config_email = ConfiguracaoEmail.objects.first()
            
            # Se CNPJ foi alterado, enviar email de notificação
...
Se você não realizou esta alteração, entre em contato imediatamente com nosso suporte."""
                ...
                try:
                    mensagem = mensagem_template.format(
                        nome=usuario.first_name,
                        novo_login=usuario.cnpj
                    )
                except:
                    mensagem = f"Seu novo login é: {usuario.cnpj}"
